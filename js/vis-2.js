...
Cartogram.prototype.wrangleData = function(){
    ...
    // Function that counts the number of states
...
Cartogram.prototype.updateVis = function(){
    ...
    var checkZero = d3.max(vis.displayData,function(d){return d.data;});

    if(d3.max(vis.displayData,function(d){return d.data;})<6){
    ...
    vis.color.domain([0,Math.log(d3.max(vis.displayData,function(d){return d.data;}))]);
    ...
    if((Math.trunc(Math.exp(quant_cutoff[0])))==1 | checkZero==0){
        ...
        vis.displayData.push({
            x:2,
            y:9.5,
            box: 0+"-"+(Math.trunc(Math.exp(quant_cutoff[0]))),
            data: 0
        });
    }
...
/*
 * Functions for Drawing Cartogram
 */
...
DropDown.prototype.createMenu = function() {
    ...
    // vis.currentDrop = null;
    // vis.dropMenu.on('change', function(){

    //     vis.currentDrop = d3.select(this)
    //         .select("select")
    //         .property("value");

    //     $(vis.eventHandler).trigger("dropdownChange", vis.currentDrop);
    // });
